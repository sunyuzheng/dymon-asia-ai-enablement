# Dymon Asia AI Enablement Speaker Notes

## Slide 1 — The Third Interface For Investment Research

Open with the big frame: AI is not just another productivity tool. It is changing the interface between human judgment and compute.

The promise for PMs: learn to use natural language to dispatch compute, tools, context, and follow-through.

## Slide 2 — Opening Question

Ask the room to guess the earlier technology in Bill Gates's framing. Let them sit with it briefly.

Most people guess internet, mobile, or cloud. That is why the question works: the right category is interface, not information network.

## Slide 3 — The Reveal

The answer is GUI. GUI changed who could operate compute.

Bridge to AI: natural language now lets more people command general-purpose work, not only operate fixed software.

## Slide 4 — PM Implication

Translate the interface shift into PM work. PMs already make judgment calls, coordinate work, and preserve context. AI raises the ceiling when those moves can be commanded, inspected, and reused.

Land the key distinction: daily work is the wedge, not the ambition. The ambition is compounding capability.

## Slide 5 — About Superlinear

Do not turn this into a bio. Let the proof points do the work: Cornell economics, Meta/Amazon/Tencent data science, Statsig experimentation, Statsig's OpenAI acquisition, AI Builders scale, and operator testimonials.

The room should infer: this training is grounded in data, experimentation, AI-native building, and teaching business operators.

## Slide 6 — Narrative Map

Walk through the five movements: interface shift, operating model, market signal, PM judgment, demo and course.

Use this slide to reassure the room that the deck will move quickly from high-level frame to concrete workflow.

## Slide 7 — Part I

State the core idea: natural language matters when it becomes a dispatch layer.

Daily work is the fastest place to learn because it repeats, has real stakes, and exposes whether the AI workflow actually saves coordination.

## Slide 8 — The Third Interface

Use the table quickly. GUI made specific tasks accessible. Programming gives generality but requires technical fluency. Natural language starts low-barrier and general-purpose.

The catch is the important part: value comes from context, tools, verification, and operating habits.

## Slide 9 — The Missing Operating Model

Use the Red Flag Act and factory analogies quickly. New technology expresses value only when work is redesigned around it.

Transition: the course is about redesigning the work loop, not collecting more AI vocabulary.

## Slide 10 — The Daily-Work Wedge

Make clear why the deck now talks about actions, opportunities, and context: these are not the grand thesis, they are the easiest measurable entry points.

Stopping leakage gives immediate value; learning to command compute is the durable skill.

## Slide 11 — Chat As Ceiling

Walk through the three failure modes: follow-through, context supply, and memory.

Land the phrase: chat keeps the human as middleware.

## Slide 12 — Agentic Operating Model

Describe the loop: specify, retrieve, act, inspect, persist. The demo will show all five steps in a simple onboarding workflow.

Transition: show that this shift is already visible in the market.

## Slide 13 — Market Signal

State the point in one sentence: leading AI-native teams redesign workflows around agents; incumbents are learning how to govern them.

## Slide 14 — Cursor / Anysphere

Use the table as a speed signal. The point is that an AI workflow can become the daily environment for a high-value profession very quickly.

## Slide 15 — Anthropic / Claude

Emphasize mastery. Strong users delegate loops, review outputs, tighten tests, and preserve learnings. Weak users stay at the chat/autocomplete layer.

## Slide 16 — Verbs, Not Nouns

Contrast vocabulary with craft. RAG, MCP, agents, and model names are useful nouns. Capability lives in verbs: specify, retrieve, delegate, inspect, test, package, rerun.

## Slide 17 — Goldman Sachs

Position Goldman as an adoption signal, not a simple success story. Even regulated incumbents are moving from experimentation to supervised agentic work.

## Slide 18 — Copy The Right Pattern

State the synthesis: workflow speed from AI-native companies, governance discipline from finance.

For Dymon, the opportunity is not copying a startup. It is importing the workflow speed while keeping source discipline and review.

## Slide 19 — Shopify And Block

Use this as operating-model proof. Shopify makes AI a management expectation; Block pushes toward intelligence layers and shared world models.

Transition: bring it back to PM work.

## Slide 20 — Part III

PM work is not mainly a text access problem. The scarce layers are filtering, mechanism, decision, sizing, and follow-through.

## Slide 21 — Investment Work, Mechanically

Walk left to right: information, signal, mechanism, judgment, action. Summarization helps at the least scarce layer; context and workflow help climb the chain.

## Slide 22 — Contrarian View

Keep the Naval point. Valuable contrarian thinking is independent reasoning under pressure to conform, not reflexive disagreement.

Land the three bullets: understand consensus, find the correct contrarian view, turn it into a tradeable decision.

## Slide 23 — Context Moves AI Up The Chain

Explain one-off research as useful coverage. Context-aware workflow moves toward judgment and action.

The competitive edge is not model access alone. It is the context architecture around judgment.

## Slide 24 — Context Architecture

Move through the rows quickly: source hierarchy, world model, variant lens, action standard.

Transition: now make it tangible with Yan's demo.

## Slide 25 — Part IV

Reset attention. The demo is intentionally ordinary because daily leakage is easiest to see in ordinary workflow.

## Slide 26 — Demo Setup

Yan should introduce the transcript in one sentence: "This is an internal onboarding process review meeting."

The first prompt is familiar: summarize and identify action items.

## Slide 27 — Demo Beat 1

Read Yan's instruction. Stress owner, counterpart, time window, meeting length, and background email.

The important observation is hands-off execution across calendar, email, and notification.

## Slide 28 — Demo Beat 2

This beat proves cross-document context. The one-pager needs the Tom onboarding case, ABC Capital KYC correspondence, and SOP 4.2.

Point out source links and the Google Doc/share/email sequence.

## Slide 29 — Demo Beat 3

Explain the context ledger: people, topics, source links, owners, derived artifacts, and status.

Use pivot examples: China energy context, Indonesian coal recurring across multiple people, and rows where no artifact exists.

## Slide 30 — Demo Standard

Judge captured work: actions, opportunities, context, hands-off execution, and transfer into Dymon workflows.

## Slide 31 — Part V

Yan's demo is the reference case. The course value is helping PMs build comparable workflows around their own research, meetings, notes, and delegation patterns.

## Slide 32 — Course Wedge

Start with one real workflow. Prove individual utility first, then package the useful pieces into reusable skills and context files.

## Slide 33 — Course Design

Session 1: complete work loop. Session 2: package judgment as context. Session 3: turn work into infrastructure.

## Slide 34 — PM AI Fluency Gaps

Walk the five gaps quickly. The most important ones for this audience are full-task delegation and compounding memory.

## Slide 35 — What Dymon Leaves With

Describe this as a first operating layer: axiom library, PM context files, reusable workflow skills, context ledger pattern, team context architecture, and support loop.

## Slide 36 — AI Is The New Excel

Use Jay's analogy to make the learning path obvious. Excel became table stakes, but real modeling required training and practice. AI has the same shape.

## Slide 37 — Closing

End on operational compounding: fewer missed actions, fewer missed opportunities, and a context system that compounds with daily work.
